...
def update_session_timeout(resp, session_id):
    max_age = 30 * 60
    resp.set_cookie('session_id', session_id, max_age = max_age)
    return session_id
...
def iterate_simulation():
    ...
    with store_lock:
        ...
        if not session_data or "sim" not in session_data or "last_interaction" not in session_data:
            simulation_store.pop(session_id, None)
            return jsonify({"error": "Simulation session corrupted or missing"}), 400
        if time.time() - session_data["last_interaction"] > 600:
            del simulation_store[session_id]
    ...
    try:
        ...
        for i in range(sim.num_neurons):
            sim_dict["neurons"][i]["vs"] = graphing_params["vs"][i]
            sim_dict["neurons"][i]["input_currents"] = graphing_params["input_currents"][i]
            sim_dict["neurons"][i]["synaptic_inputs"] = graphing_params["synaptic_inputs"][i]
...
def set_current():
    ...
    try:
        ...
        for neuron_index in data:
            ...
            neuron = sim.neuron_models[int(neuron_index)]
...
def setup_simulation():
    ...
    if not session_id:
        ...
        session_id = update_session_timeout(resp, session_id)
